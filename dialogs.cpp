    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            st->edit = make(L"EDIT", st->text.c_str(),
                            WS_TABSTOP | WS_BORDER | ES_AUTOHSCROLL, 12, 34, 340, 24, hwnd, 100);
...
struct view_state { HWND edit = nullptr; };
...
LRESULT CALLBACK view_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            SetWindowLongPtrW(hwnd, GWLP_USERDATA, (LONG_PTR)st);
            st->edit = make(L"EDIT", L"", WS_BORDER | WS_VSCROLL | WS_HSCROLL |
        ...
        case WM_SIZE: {
            ...
            if (close) MoveWindow(close, rc.right - 92, rc.bottom - 32, 80, 26, TRUE);
            return 0;
        }
        case WM_COMMAND:
...
void add_preview(HWND hwnd, add_state* st) {
    ...
    for (; it != end; it.increment(ec)) {
        ...
        if (entry.is_directory(ec)) {
            for (auto& pat : comp) {
                if (fnmatch(pat, name)) { it.disable_recursion_pending(); break; }
            }
            continue;
        }
        ...
        // component patterns vs each path part
        {
            ...
            for (auto& pat : comp) {
                for (auto& part : parts)
                    if (fnmatch(pat, part)) { skip = true; break; }
                if (skip) break;
            }
        }
        if (!skip) {
            ...
            for (auto& pat : path_pats)
                if (fnmatch(pat, norm)) { skip = true; break; }
        }
        if (skip) ++ignored;
        else watched.push_back(relw);
    }

    std::wstring body;
    for (auto& w : watched) { body += w; body += L"\r\n"; }
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            st->patterns = make(L"LISTBOX", L"",
                                ...
                                12, 160, 484, 220, hwnd, 400);

            make(L"BUTTON", L"Add...", WS_TABSTOP, 12, 388, 80, 26, hwnd, 401);
            make(L"BUTTON", L"Remove", WS_TABSTOP, 98, 388, 80, 26, hwnd, 402);

            st->preview_btn = make(L"BUTTON", L"Preview Files...", WS_TABSTOP, 12, 426, 130, 26, hwnd, 403);
        ...
        case WM_COMMAND: {
            ...
            if (id >= 300 && id < 300 + (int)g_presets.size()) {
                add_rebuild_from_presets(st);
                return 0;
            }
            if (id == 401) { // Add pattern
                std::wstring text;
            ...
            if (id == 403) { add_preview(hwnd, st); return 0; }
            if (id == IDOK) {
                if (!st->have_path) return 0;
                st->result = listbox_get_utf8(st->patterns);
                st->skip_binary = (SendMessageW(GetDlgItem(hwnd, 500), BM_GETCHECK, 0, 0) == BST_CHECKED);
                st->ok = true;
                DestroyWindow(hwnd);
                return 0;
            }
            if (id == IDCANCEL) { st->ok = false; DestroyWindow(hwnd); return 0; }
            break;
        }
    }
    return DefWindowProcW(hwnd, msg, wp, lp);
...
LRESULT CALLBACK ignore_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            for (size_t i = 0; i < g_presets.size(); ++i) {
                make(L"BUTTON", to_wide(g_presets[i].first).c_str(),
                     WS_TABSTOP, px, 10, 90, 26, hwnd, 300 + (int)i);
                px += 96;
            }
            make(L"STATIC",
                 L"Patterns without / match any path component.\r\n"
            ...
            st->patterns = make(L"LISTBOX", L"",
                                WS_TABSTOP | WS_BORDER | WS_VSCROLL | LBS_NOTIFY,
                                12, 100, 434, 230, hwnd, 400);
            make(L"BUTTON", L"Add...", WS_TABSTOP, 12, 338, 80, 26, hwnd, 401);
            make(L"BUTTON", L"Remove", WS_TABSTOP, 98, 338, 80, 26, hwnd, 402);
        ...
        case WM_COMMAND: {
            int id = LOWORD(wp);
            if (id >= 300 && id < 300 + (int)g_presets.size()) {
                ignore_add_preset(st, g_presets[id - 300].first);
                return 0;
            }
            if (id == 401) {
                std::wstring text;
...
void show_diff(HWND rich, const std::string& diff) {
    ...
    while (std::getline(ss, line)) {
        ...
        if (line.rfind("+++", 0) == 0 || line.rfind("---", 0) == 0) col = COL_HEADER;
        else if (line.rfind("@@", 0) == 0) col = COL_HUNK;
        else if (!line.empty() && line[0] == '+') col = COL_ADD;
        else if (!line.empty() && line[0] == '-') col = COL_DEL;
...
void restore_pick_font(HWND hwnd, restore_state* st) {
    ...
    if (!ChooseFontW(&cf))
        return;
    if (st->preview_font) DeleteObject(st->preview_font);
    st->preview_font = CreateFontIndirectW(&st->font);
    set_font(st->preview, st->preview_font);

    json::value cfg;
    if (!load_config(cfg)) cfg = json::value::make_object();
...
void restore_load_font(HWND hwnd, restore_state* st) {
    ...
    if (load_config(cfg)) {
        if (auto* f = cfg.find("restore_font")) {
            if (auto* fam = f->find("family")) family = fam->as_string("Consolas");
            if (auto* sz = f->find("size")) size = sz->as_int(9);
        }
    }
    st->font = {};
    HDC dc = GetDC(hwnd);
...
void restore_layout(HWND hwnd, restore_state* st) {
    ...
    int left_w = 240;
    int mid_w = 360;
    int right_x = margin + left_w + margin + mid_w + margin;
    int right_w = W - right_x - margin;
    if (right_w < 120) right_w = 120;

    // left: label + commits
    HWND lbl_snap = GetDlgItem(hwnd, 500);
    if (lbl_snap) MoveWindow(lbl_snap, margin, top, left_w, 18, TRUE);
    MoveWindow(st->commits_lb, margin, top + 22, left_w, body_h - 22, TRUE);

    // mid: mode row + buttons + list
    int mx = margin + left_w + margin;
    MoveWindow(st->mode_changed, mx, top, 110, 22, TRUE);
    MoveWindow(st->mode_all, mx + 112, top, 110, 22, TRUE);
    MoveWindow(GetDlgItem(hwnd, 504), mx + mid_w - 150, top, 72, 24, TRUE); // Select All
    MoveWindow(GetDlgItem(hwnd, 505), mx + mid_w - 74, top, 74, 24, TRUE);  // Select None
    ...
    MoveWindow(GetDlgItem(hwnd, 508), right_x + right_w - 80, top, 80, 24, TRUE); // Font
...
LRESULT CALLBACK restore_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            make(L"STATIC", L"Snapshots", 0, 0, 0, 10, 10, hwnd, 500);
            st->commits_lb = make(L"LISTBOX", L"",
                                  WS_TABSTOP | WS_BORDER | WS_VSCROLL | LBS_NOTIFY,
                                  0, 0, 10, 10, hwnd, 501);

            st->mode_changed = make(L"BUTTON", L"Changed files",
                                    WS_TABSTOP | WS_GROUP | BS_AUTORADIOBUTTON, 0, 0, 10, 10, hwnd, 502);
            st->mode_all = make(L"BUTTON", L"Full snapshot",
                                BS_AUTORADIOBUTTON, 0, 0, 10, 10, hwnd, 503);
            SendMessageW(st->mode_changed, BM_SETCHECK, BST_CHECKED, 0);
            make(L"BUTTON", L"Select All", WS_TABSTOP, 0, 0, 10, 10, hwnd, 504);
            make(L"BUTTON", L"Select None", WS_TABSTOP, 0, 0, 10, 10, hwnd, 505);

            st->files_lv = CreateWindowExW(0, WC_LISTVIEWW, L"",
                WS_CHILD | WS_VISIBLE | WS_TABSTOP | WS_BORDER | LVS_REPORT | LVS_SINGLESEL,
                0, 0, 10, 10, hwnd, (HMENU)(INT_PTR)506, GetModuleHandleW(nullptr), nullptr);
            ...
            st->view_content = make(L"BUTTON", L"Content",
                                    WS_TABSTOP | WS_GROUP | BS_AUTORADIOBUTTON, 0, 0, 10, 10, hwnd, 507);
            st->view_diff = make(L"BUTTON", L"Diff", BS_AUTORADIOBUTTON, 0, 0, 10, 10, hwnd, 509);
            SendMessageW(st->view_content, BM_SETCHECK, BST_CHECKED, 0);
            make(L"BUTTON", L"Font...", WS_TABSTOP, 0, 0, 10, 10, hwnd, 508);

            st->preview = CreateWindowExW(0, MSFTEDIT_CLASS, L"",
                WS_CHILD | WS_VISIBLE | WS_BORDER | WS_VSCROLL | WS_HSCROLL |
                ES_MULTILINE | ES_READONLY | ES_AUTOVSCROLL | ES_AUTOHSCROLL,
                0, 0, 10, 10, hwnd, (HMENU)(INT_PTR)510, GetModuleHandleW(nullptr), nullptr);
            ...
            st->count_lbl = make(L"STATIC", L"", 0, 0, 0, 10, 10, hwnd, 511);
            st->export_btn = make(L"BUTTON", L"Export Selected", WS_TABSTOP, 0, 0, 10, 10, hwnd, 512);
        ...
        case WM_SIZE:
            if (st) restore_layout(hwnd, st);
            return 0;
        case WM_NOTIFY: {
            auto* nm = reinterpret_cast<NMHDR*>(lp);
            if (nm->idFrom == 506 && nm->code == LVN_ITEMCHANGED) {
        ...
        case WM_COMMAND: {
            ...
            if (id == 502 || id == 503) { restore_refresh_files(st); return 0; }
            if (id == 507 || id == 509) { restore_refresh_preview(st); return 0; }
            if (id == 504) { // select all
                int n = ListView_GetItemCount(st->files_lv);
                for (int i = 0; i < n; ++i) ListView_SetCheckState(st->files_lv, i, TRUE);
                return 0;
            }
            if (id == 505) {
                int n = ListView_GetItemCount(st->files_lv);
                for (int i = 0; i < n; ++i) ListView_SetCheckState(st->files_lv, i, FALSE);
                return 0;
            }
            if (id == 508) { restore_pick_font(hwnd, st); return 0; }
            if (id == 512) { restore_export(hwnd, st); return 0; }
        ...
        case WM_DESTROY:
            ...
            if (st && st->preview_font) DeleteObject(st->preview_font);
            return 0;
    }
    return DefWindowProcW(hwnd, msg, wp, lp);
}
