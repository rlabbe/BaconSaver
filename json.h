...
struct value {
    enum kind { null_t, bool_t, num_t, str_t, arr_t, obj_t } type = null_t;
    bool b = false;
    double num = 0;
    std::string str;
    ...
    static value make_array() { value v; v.type = arr_t; v.arr = std::make_shared<array>(); return v; }
    static value make_object() { value v; v.type = obj_t; v.obj = std::make_shared<object>(); return v; }
    ...
    const value* find(const std::string& key) const {
        if (type != obj_t || !obj) return nullptr;
        for (auto& kv : *obj)
    ...
    void set(const std::string& key, value v) {
        if (type != obj_t) { type = obj_t; obj = std::make_shared<object>(); }
        for (auto& kv : *obj)
    ...
    std::string as_string(const std::string& def = "") const {
        return type == str_t ? str : def;
    }
    int as_int(int def = 0) const {
        return type == num_t ? (int)std::llround(num) : def;
    }
    bool as_bool(bool def = false) const {
        return type == bool_t ? b : def;
    }
};
...
struct parser {
    ...
    bool parse_value(value& out) {
        ...
        if (c == 't') {
            if (end - p >= 4 && strncmp(p, "true", 4) == 0) { p += 4; out = value(true); return true; }
            return false;
        }
        if (c == 'f') {
            if (end - p >= 5 && strncmp(p, "false", 5) == 0) { p += 5; out = value(false); return true; }
            return false;
        }
        if (c == 'n') {
            if (end - p >= 4 && strncmp(p, "null", 4) == 0) { p += 4; out = value(); return true; }
            return false;
        }
        // number
        char* numend = nullptr;
...
inline void dump_value(const value& v, std::string& out, int indent, int depth) {
    ...
    switch (v.type) {
        case value::null_t: out += "null"; break;
        case value::bool_t: out += v.b ? "true" : "false"; break;
        case value::num_t: {
            double d = v.num;
        ...
        case value::str_t: dump_string(v.str, out); break;
        case value::arr_t: {
            if (!v.arr || v.arr->empty()) { out += "[]"; break; }
            out += "[\n";
        ...
        case value::obj_t: {
            if (!v.obj || v.obj->empty()) { out += "{}"; break; }
            out += "{\n";
